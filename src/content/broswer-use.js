...
function simulateClick(xpath) {
    ...
    let targetElement = isClickable(element) ? element : findClickableParent(element);
...
async function simulateInput(xpath, text) {
    try {
        ...
        const isAntSelect = element.classList && (
            element.classList.contains('ant-select-selection-search-input') ||
            element.id && element.id.startsWith('rc_select')
        );

        const selectRoot = isAntSelect ? (element.closest('.ant-select') || element.parentElement) : null;
...
function simulateKeypress(options) {
    try {
        ...
        eventTypes.forEach(eventType => {
            ...
            if (eventType === 'keypress' && ['Tab', 'Escape', 'ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(keyInfo.key)) {
                return;
            }
        ...
        // 处理特殊按键的默认行为
        if (keyName === 'enter' && targetElement.form && targetElement.type !== 'textarea') {
            // 如果是表单中的输入框，回车可能需要提交表单
            console.log('检测到表单元素的回车键，可能需要提交表单');
        } else if (keyName === 'space' && (targetElement.tagName === 'BUTTON' || targetElement.role === 'button')) {
            // 如果是按钮，空格键可能需要触发点击
            targetElement.click();
        }
...
function scrollPage(options) {
    try {
        ...
        if (options.xpath) {
            ...
            // 滚动到页面底部
            window.scrollTo({
                top: (document.documentElement.scrollHeight || document.body.scrollHeight) - window.innerHeight,
                behavior: options.smooth ? 'smooth' : 'auto'
            });
